 ...
 * @author potter
 */
public abstract class SEXYChart<X,Y> extends SEChart {
    ...
    private final ListChangeListener<Series<X,Y>> seriesChanged = c -> {
        ...
        while (c.next()) {
            ...
            Set<Series<X, Y>> dupCheck = new LinkedHashSet<>(displayedSeries);
    ...
    private BooleanProperty verticalGridLinesVisible = new StyleableBooleanProperty(true) {
        ...
        @Override
        public CssMetaData<SEXYChart<?,?>,Boolean> getCssMetaData() {
     ...
     */
    public SEXYChart(Axis<X> xAxis, Axis<Y> yAxis) {
        this.xAxis = xAxis;
...
//
    ...
    private static class StyleableProperties {
        private static final CssMetaData<SEXYChart<?,?>,Boolean> HORIZONTAL_GRID_LINE_VISIBLE =
            new CssMetaData<SEXYChart<?,?>,Boolean>("-fx-horizontal-grid-lines-visible",
                BooleanConverter.getInstance(), Boolean.TRUE) {

            @Override
            public boolean isSettable(SEXYChart<?,?> node) {
                return node.horizontalGridLinesVisible == null ||
                        !node.horizontalGridLinesVisible.isBound();
            }

            @Override
            public StyleableProperty<Boolean> getStyleableProperty(SEXYChart<?,?> node) {
                return (StyleableProperty<Boolean>)(WritableValue<Boolean>)node.horizontalGridLinesVisibleProperty();
            }
        };

        private static final CssMetaData<SEXYChart<?,?>,Boolean> HORIZONTAL_ZERO_LINE_VISIBLE =
            new CssMetaData<SEXYChart<?,?>,Boolean>("-fx-horizontal-zero-line-visible",
                BooleanConverter.getInstance(), Boolean.TRUE) {
        ...
        private static final CssMetaData<SEXYChart<?,?>,Boolean> ALTERNATIVE_ROW_FILL_VISIBLE =
            new CssMetaData<SEXYChart<?,?>,Boolean>("-fx-alternative-row-fill-visible",
                BooleanConverter.getInstance(), Boolean.TRUE) {
        ...
        private static final CssMetaData<SEXYChart<?,?>,Boolean> VERTICAL_GRID_LINE_VISIBLE =
            new CssMetaData<SEXYChart<?,?>,Boolean>("-fx-vertical-grid-lines-visible",
                BooleanConverter.getInstance(), Boolean.TRUE) {
        ...
        private static final CssMetaData<SEXYChart<?,?>,Boolean> VERTICAL_ZERO_LINE_VISIBLE =
            new CssMetaData<SEXYChart<?,?>,Boolean>("-fx-vertical-zero-line-visible",
                BooleanConverter.getInstance(), Boolean.TRUE) {
        ...
        private static final CssMetaData<SEXYChart<?,?>,Boolean> ALTERNATIVE_COLUMN_FILL_VISIBLE =
            new CssMetaData<SEXYChart<?,?>,Boolean>("-fx-alternative-column-fill-visible",
                BooleanConverter.getInstance(), Boolean.TRUE) {
        ...
        static {
            final List<CssMetaData<? extends Styleable, ?>> styleables =
                new ArrayList<CssMetaData<? extends Styleable, ?>>(SEChart.getClassCssMetaData());
    ...
    public final static class Data<X,Y> {
        ...
        private ObjectProperty<X> xValue = new ObjectPropertyBase<X>() {
            @Override protected void invalidated() {
                ...
                get();
                if (series!=null) {
                    SEXYChart<X,Y> chart = series.getChart();
    ...
    public static final class Series<X,Y> {
        ...
        private final ListChangeListener<Data<X,Y>> dataChangeListener = new ListChangeListener<Data<X, Y>>() {
            @Override public void onChanged(ListChangeListener.Change<? extends Data<X, Y>> c) {
                ObservableList<? extends Data<X, Y>> data = c.getList();
                final SEXYChart<X, Y> chart = getChart();
                while (c.next()) {
                    if (chart != null) {
                        ...
                        Set<Data<X, Y>> dupCheck = new LinkedHashSet<>(displayedData);
                        dupCheck.removeAll(c.getRemoved());
                        ...
                        Set<Data<X, Y>> dupCheck = new LinkedHashSet<>();
                        for (Data<X, Y> d : data) {
                            if (!dupCheck.add(d)) {
                                throw new IllegalArgumentException("Duplicate data added");
                            }
        ...
        private final ReadOnlyObjectWrapper<SEXYChart<X,Y>> chart = new ReadOnlyObjectWrapper<SEXYChart<X,Y>>(this, "chart") {
            @Override
            protected void invalidated() {
                if (get() == null) {
                    displayedData.clear();
        ...
        public final SEXYChart<X,Y> getChart() { return chart.get(); }
        private void setChart(SEXYChart<X,Y> value) { chart.set(value); }
        public final ReadOnlyObjectProperty<SEXYChart<X,Y>> chartProperty() { return chart.getReadOnlyProperty(); }
